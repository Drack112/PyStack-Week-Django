...
def home(request):
    if request.session.get("usuario"):
        ...
        return render(
            request, "home.html", {"cursos": cursos, "request_usuario": request_usuario}
        )
    else:
...
def curso(request, id):
    if request.session.get("usuario"):
        ...
        request_usuario = request.session.get("usuario")
        return render(
            request, "curso.html", {"aulas": aulas, "request_usuario": request_usuario}
        )
    else:
        return redirect("/auth/login/?status=2")
    ...
    if request.session.get("usuario"):
        ...
        if not usuario_avaliou:
            nota_aulas = NotasAulas(
                aula_id=aula_id, nota=avaliacao, usuario_id=usuario_id
            )
            nota_aulas.save()
